...
def generatePath(s0,mu,sig,t,n=10000,method = ""):
	...
	if(method=="d"):
		...
		for i in range(len(times)):
			Y.append(math.exp(np.random.uniform()))
			X.append(np.random.normal())
		c = -np.cov(X,Y)[1][0]/np.var(Y)
		prices = []
		mu = np.mean(Y)
		for i in range(len(times)):
			X[i] = X[i] + c*(Y[i]-mu)
			current = current*math.exp(((X[i]*(delta**0.5))*(sig)) + ((mu-(sig*sig/2))*delta))
...
for i in range(10):
	plt.plot(np.arange(0,252),generatePath(100,0.1,0.2,1,252))
plt.xlabel("t")
plt.ylabel("X")
plt.title("GBM paths for real world measure")
plt.show()

for i in range(10):
	plt.plot(np.linspace(0,5,10000),generatePath(100,0.05,0.2,5,10000))

plt.xlabel("t")
plt.ylabel("X")
plt.title("GBM paths for risk neutral measure")
plt.show()
...
for K in [105,110,90]:
	print("\n\nFor K = ",K)
	m = getOptionPrice(K,100,0.05,0.2,0.5,126,1000)
...
for K in k:
	price = getOptionPrice(K,100,0.05,0.2,0.5,126,1000)
...
for r in R:   
	price = getOptionPrice(105,100,r,0.2,0.5,126,1000)
	callPrices.append(price[0])
	putPrices.append(price[2])
...
for sig in SIG:
	price = (getOptionPrice(105,100,0.05,sig,0.5,126,1000))
	callPrices.append(price[0])
	putPrices.append(price[2])
...
S = np.arange(50,151)
for s in S:
	price = getOptionPrice(105,s,0.05,0.2,0.5,126,1000)
	callPrices.append(price[0])
	putPrices.append(price[2])
...
for t in T:
	price = getOptionPrice(105,100,0.05,0.2,t,int(252*t),1000)
	callPrices.append(price[0])
	putPrices.append(price[2])
