...
def qqplot(df,interval):
	for j in range(5):
		...
		for i in range(4):
			df1 = pd.DataFrame()
			df1[companies[4*j+i+2]] = df.loc[:,companies[4*j+i+2]]
			df1= df1.sort_values(by=[companies[4*j+i+2]],ascending=True).reset_index(drop=True)
			df1['count'] = df1.index + 1;
			df1['comparison_points'] = (df1[companies[4*j+i+2]] - df1[companies[4*j+i+2]].mean())/df1[companies[4*j+i+2]].std(ddof=0)
